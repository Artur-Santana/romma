...
export default function GestaoEdificios() {
  ...
  return (
    <div className="romma-page p-12 bg-background min-h-full">
          ...
          <form onSubmit={handleCriar}>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <label className="flex flex-col gap-1.5">
                <span className="font-mono text-[13px] tracking-[1px] uppercase text-fg-4">Nome</span>
                <Input
                  type="text"
              ...
              <label className="flex flex-col gap-1.5">
                <span className="font-mono text-[13px] tracking-[1px] uppercase text-fg-4">Endereço</span>
                <Input
                  type="text"
            ...
            {erro && (
              <div className="bg-[var(--danger-bg2)] border-l-2 border-l-danger-fg px-4 py-3 font-mono text-[15px] text-danger-fg mb-4">
                {erro}
              </div>
            )}
            ...
            <Button
              ...
              disabled={loading}
              className={cn(
                "bg-indigo text-fg-1 font-body font-bold text-[18px] tracking-[1.2px] uppercase px-10 py-[18px] rounded-none mt-2",
      ...
      {erro && !showForm && (
        <div className="bg-[var(--danger-bg2)] border-l-2 border-l-danger-fg px-4 py-3 font-mono text-[16px] text-danger-fg mb-4">
          {erro}
        </div>
      )}
      ...
      <div className="flex flex-col gap-0 border border-border-3 bg-surface">
        {edificios.length === 0 && (
          <p className="px-5 py-4 font-mono text-[15px] text-fg-5">Nenhum edifício cadastrado.</p>
        )}
          ...
          >
            {editandoId === edificio.id ? (
              <div className="flex flex-col gap-3">
                ...
                {erroEdit && (
                  <div className="bg-[var(--danger-bg2)] border-l-2 border-l-danger-fg px-4 py-3 font-mono text-[15px] text-danger-fg">
                    {erroEdit}
                  </div>
                )}
                <div className="flex gap-2 mt-1">
                  <Button
                    ...
                    disabled={loading}
                    className="bg-indigo text-fg-1 font-body font-bold text-[16px] tracking-[1.2px] uppercase px-7 py-3 rounded-none cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="font-mono text-[14px] text-fg-4 px-4 py-2 border border-border-3 hover:text-fg-1"
                  >
                    Cancelar
                  </button>
                </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-body font-bold text-[25px] text-fg-1">{edificio.nome}</p>
                  <p className="font-mono text-[23px] text-fg-4 mt-0.5 tracking-[0.3px]">{edificio.endereco}</p>
                </div>
                <div className="flex gap-2 shrink-0">
                  <button
                    onClick={() => handleEditar(edificio)}
                    style={{ all: "unset", cursor: "pointer" }}
                    className="font-mono text-[14px] text-fg-3 px-3 py-1.5 border border-border-3 hover:text-fg-1 hover:border-border-1"
                  >
                    Editar
                  </button>
                  <button
                    ...
                    style={{ all: "unset", cursor: "pointer" }}
                    className="font-mono text-[14px] text-danger-fg px-3 py-1.5 border border-danger-fg hover:bg-[var(--danger-bg2)]"
                  >
                    Remover
                  </button>
                </div>
