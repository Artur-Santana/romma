    ...
    <div className="romma-desktop-only romma-page p-12 bg-background min-h-full">
      ...
      <div className="bg-surface border border-border-3">
        ...
        <div style={{ display: "grid", gridTemplateColumns: GRID }} className="px-5 py-3 bg-[var(--surface-hi)]">
          {["Nome", "Tipo", "Documento", "Email", "Contratos", "Status", "Ações"].map(h => (
            <span key={h} className="font-mono text-[11px] font-bold tracking-[1.5px] uppercase text-fg-4">{h}</span>
          ))}
        </div>

        {/* Erro de revogar inline — abaixo do header */}
        {erro && (
          <div className="px-5 py-2 font-mono text-[14px] text-danger-fg border-t border-border-3">{erro}</div>
        )}

        {/* Rows */}
        {locatarios.length === 0 && (
          <div className="px-5 py-8 font-mono text-[15px] text-fg-4 text-center">
          ...
          return (
            <div key={l.id} style={{ display: "grid", gridTemplateColumns: GRID }} className={cn("px-5 py-4 items-center", i > 0 ? "border-t border-border-3" : "")}>
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div className={cn("w-8 h-8 shrink-0 flex items-center justify-center border font-body font-bold text-[13px] tracking-[1px]", isPendente ? "bg-transparent border-border-2 text-fg-4" : "bg-surface border-border-2 text-fg-1")}>
                  {ini}
                </div>
                <span className="font-body font-medium text-[23px] text-fg-1 overflow-hidden text-ellipsis whitespace-nowrap">{l.nome_razao_social}</span>
              </div>

              {/* Tipo */}
              <span className="font-mono text-[13px] tracking-[0.5px] text-fg-3">
                {l.tipo?.toUpperCase()}
              </span>

              {/* Documento */}
              <span className="font-mono text-[23px] text-fg-2">
                {fmtDoc(l.tipo, l.documento)}
              </span>

              {/* Email */}
              <span className="font-mono text-[23px] text-fg-3 overflow-hidden text-ellipsis whitespace-nowrap">{l.email}</span>
      ...
      <div className="mt-8 p-6 border border-indigo flex justify-between items-center bg-[oklch(0.339_0.179_301.68_/_0.06)]">
        <div className="flex flex-col gap-1.5">
          ...
          <span className="text-[16px] text-fg-2 leading-relaxed max-w-[540px]">
        ...
        <Button
          ...
          className="bg-indigo text-fg-1 font-mono font-bold text-[14px] tracking-[1.4px] uppercase rounded-none"
        >Convidar →</Button>
      </div>
        ...
        >
          <div className="bg-surface border border-border-2 w-[480px] p-8 flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              ...
              <h3 className="font-body font-bold text-[38px] text-fg-1 m-0">
                Editar Locatário
              </h3>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              {erro && (
                <span className="font-mono text-[14px] text-danger-fg">{erro}</span>
              )}
              ...
              <div className="flex gap-3 justify-end mt-2">
                <Button
                  ...
                  className="text-fg-3 font-mono text-[18px] border border-border-3 rounded-none px-5 py-[10px] h-auto"
                >Cancelar</Button>
                <Button
                  type="button"
                  disabled={loading}
                  ...
                  className={cn(
                    "bg-indigo text-fg-1 font-body font-bold text-[16px] tracking-[1.2px] uppercase px-8 py-[14px] rounded-none h-auto",
                ...
                <div className="flex gap-0">
                  {["pf", "pj"].map(t => (
                    <button
                      ...
                      className={cn(
                        "cursor-pointer py-2 px-5 font-mono font-bold text-[14px] tracking-[1px] uppercase border border-border-3",
...
function Field({ label, required, children }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="font-mono text-[13px] font-bold tracking-[1px] uppercase text-fg-4">
