  ...
  return (
    <div className="romma-page bg-background min-h-full px-12 pt-12 pb-20">
      ...
      <Button
        ...
        className="border-border-3 bg-transparent text-fg-3 font-mono text-[15px] uppercase tracking-[1.2px] font-bold rounded-none mb-10 h-auto py-[10px] px-5"
      >
        ← Contratos
      </Button>
      ...
      <div className="flex flex-col gap-3 mb-12">
        ...
        <h2 className="font-display font-bold text-[72px] leading-none tracking-[-2.4px] text-fg-1 m-0">
          Parcelas.
        </h2>
        {(locatario || unidade) && (
        ...
        <span className="font-mono text-[27px] text-fg-4">
          {pagas} pagas · {pendentes} pendentes · {parcelas.length} total
        </span>
      </div>

      {erro && (
        <div className="px-4 py-[10px] mb-6 bg-[var(--danger-bg2)] border border-danger-fg font-mono text-[18px] text-danger-fg">
          {erro}
        </div>
      )}
        ...
        {parcelas.map((parcela, i) => (
          ...
          >
            <div className="px-5 py-4">
              <span className="font-mono text-[27px] text-fg-2 font-bold">
            ...
            <div className="px-5 py-4">
              <span className={cn("font-mono text-[27px]", parcela.status === "vencida" ? "text-danger-fg" : "text-fg-3")}>
                {fmtData(parcela.data_vencimento)}
              </span>
            </div>

            <div className="px-5 py-4">
              <span className={cn("font-mono text-[27px]", parcela.data_pagamento ? "text-success" : "text-fg-5")}>
            ...
            <div className="px-5 py-4">
              {(parcela.status === "pendente" || parcela.status === "vencida") && (
                <Button
                  ...
                  className="border-success text-success font-mono text-[18px] uppercase tracking-[1px] font-bold rounded-none h-auto py-3 px-5"
                >
                  Marcar Paga
                </Button>
              )}
