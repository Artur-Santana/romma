      ...
      >
        ...
        <div className="flex justify-between items-start">
          <div className="flex flex-col gap-1">
            <span className="font-mono text-[17px] text-fg-5 tracking-[1px] uppercase">
              {refOf(unidade)}
            </span>
            <h2 className="font-body font-bold text-[48px] tracking-[-1.6px] text-fg-1 leading-none m-0">
              {unidade.nome}
            </h2>
            {edificio && (
              <span className="text-[20px] text-fg-3">{edificio.nome}</span>
            )}
          </div>
          <button
            style={{ all: 'unset', cursor: 'pointer', display: 'flex', width: 44, height: 44, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
            className="border border-border-3 text-fg-3 text-[21px]"
            onClick={onClose}
          >
            ✕
          </button>
        ...
        <div className="border border-border-3 grid grid-cols-2">
          <div className="p-4 flex flex-col gap-1.5">
            <span className="font-mono text-[17px] text-fg-5 tracking-[1px] uppercase">Área</span>
            <span className="font-body font-bold text-[33px] tracking-[-0.8px] text-fg-1">
              {unidade.area_m2 != null ? `${unidade.area_m2}m²` : '—'}
            </span>
          </div>
          <div className="p-4 border-l border-border-3 flex flex-col gap-1.5">
            <span className="font-mono text-[17px] text-fg-5 tracking-[1px] uppercase">Valor Mensal</span>
            <span className="font-body font-bold text-[33px] tracking-[-0.8px] text-fg-1">
        ...
        {unidade.descricao && (
          <p className="font-body text-[20px] text-fg-2 leading-[1.55] m-0">
            {unidade.descricao}
          </p>
        )}
        ...
        {edificio?.endereco && (
          <div className="bg-surface border border-border-3 px-4 py-3 flex justify-between items-center gap-3">
            <span className="font-mono text-[17px] text-fg-5 tracking-[1px] uppercase shrink-0">
              Endereço
            </span>
            <span className="text-[18px] text-fg-3 text-right">
              {edificio.endereco}
            </span>
          </div>
        )}
        ...
        <div className="flex flex-col gap-2">
          <button
            ...
            className="py-[14px] px-5 bg-indigo font-body font-bold text-[20px] text-fg-1 text-center tracking-[0.5px] min-h-[44px]"
          ...
          <button
            style={{ all: 'unset', cursor: 'pointer', display: 'block', width: '100%', boxSizing: 'border-box', minHeight: 44 }}
            className="py-[14px] px-5 border border-border-3 font-body font-bold text-[20px] text-fg-3 text-center min-h-[44px]"
            onClick={onClose}
          >
            Fechar
          </button>
        ...
        <span className="font-mono text-[17px] text-fg-5 text-center tracking-[0.5px]">
