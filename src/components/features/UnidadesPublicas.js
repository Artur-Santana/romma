    ...
    <div className="bg-background h-dvh flex flex-col relative overflow-hidden">
      <div className="px-5 pt-5 pb-6 border-b border-border-3 flex flex-col gap-2">
        <div className="flex justify-between items-center">
          <Link href="/" className="font-mono text-[17px] text-fg-4 tracking-[1px] uppercase hover:text-fg-2 transition-colors py-3 inline-flex items-center min-h-[44px]">← Voltar</Link>
          <RealtimeDot />
        </div>
        <h1 className="font-body font-bold text-[48px] tracking-[-1.6px] text-fg-1 leading-none m-0 whitespace-pre-line">
          {'Unidades\nDisponíveis.'}
        </h1>
      </div>
        ...
        {tabs.map(tab => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-3 min-h-[44px] inline-flex gap-2 font-body font-bold text-[15px] uppercase tracking-[0.5px] items-center border ${
                isActive
      ...
      <div className="px-5 py-4 border-b border-border-3 flex justify-between items-baseline">
        <span className="font-mono text-[17px] text-fg-4 tracking-[0.5px] opacity-50">
      ...
      <div className="flex-1 overflow-auto">
        {filtered.length === 0 ? (
          <div className="py-20 px-8 text-center flex flex-col gap-3 items-center">
            <div className="w-12 h-12 border border-border-3 flex items-center justify-center text-[27px] text-fg-4">
              —
            </div>
            <span className="font-body font-bold text-[33px] tracking-[-0.8px] text-fg-2 block">
              Nenhuma unidade disponível
            </span>
            <p className="text-[18px] text-fg-4 leading-[1.5] max-w-[240px] m-0">
        ...
        <div className="py-8 px-5 text-center border-t border-border-3">
          <span className="font-mono text-[17px] text-fg-5 tracking-[1.5px]">
