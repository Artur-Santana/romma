...
function HeaderCell({ children }) {
  return (
    <div className="px-5 py-3 font-mono text-[15px] font-bold tracking-[1.4px] uppercase text-fg-4">
      {children}
    </div>
  )
}
    ...
    <>
      ...
      <div className="romma-page px-12 pt-12 pb-20 bg-background min-h-full">
            ...
            <form onSubmit={handleCriarContrato}>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <label className="flex flex-col gap-1.5">
                  <span className="font-mono text-[15px] tracking-[1px] text-fg-4 uppercase">Locatário</span>
                  <Select
                    required
                ...
                <label className="flex flex-col gap-1.5">
                  <span className="font-mono text-[15px] tracking-[1px] text-fg-4 uppercase">Unidade (disponíveis)</span>
                  <Select
                    required
                ...
                <label className="flex flex-col gap-1.5">
                  <span className="font-mono text-[15px] tracking-[1px] text-fg-4 uppercase">Data de Início</span>
                  <Input
                    required
                    type="date"
                ...
                <label className="flex flex-col gap-1.5">
                  <span className="font-mono text-[15px] tracking-[1px] text-fg-4 uppercase">Data de Término</span>
                  <Input
                    required
                    type="date"
                    value={form.data_fim}
              ...
              {unidadeSelecionada && (
                <div className="px-4 py-3 mb-4 border border-border-3 bg-surface-hi flex items-center gap-4">
                  <span className="font-mono text-[15px] text-fg-4 tracking-[1px] uppercase">Valor Mensal</span>
                  <span className="font-mono text-[27px] text-fg-1 font-bold">
              ...
              <label className="flex flex-col gap-1.5 mb-5">
                <span className="font-mono text-[15px] tracking-[1px] text-fg-4 uppercase">Observações</span>
                <textarea
                  rows={3}
                  value={form.observacoes}
                  onChange={e => setForm({ ...form, observacoes: e.target.value })}
                  placeholder="Opcional"
                  className="bg-surface-hi border border-border-3 text-fg-1 font-body text-[24px] px-3.5 py-2.5 w-full resize-y focus:outline-none"
                />
              </label>

              {erro && (
                <div className="bg-[var(--danger-bg2)] border border-[var(--danger-bg)] border-l-2 border-l-danger-fg px-4 py-3 font-mono text-[18px] text-danger-fg mb-4">
                  {erro}
                </div>
              )}
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={loading}
                  className={cn(
                    "bg-indigo text-fg-1 font-body font-bold text-[21px] tracking-[1.2px] uppercase px-10 py-[18px] rounded-none",
                ...
                <Button
                  ...
                  className="border border-border-3 bg-transparent text-fg-3 font-body font-bold text-[21px] tracking-[1.2px] uppercase px-8 py-[18px] rounded-none cursor-pointer"
                >
                  Cancelar
                </Button>
              </div>
        ...
        <div className="border border-border-3 bg-surface mb-8">
          ...
          {contratos.length === 0 && (
            <div className="py-12 px-5 text-center font-mono text-[18px] text-fg-4 tracking-[0.5px]">
            ...
            return (
              ...
              >
                <div className="px-5 py-4">
                  <span className="font-mono text-[27px] text-fg-4 tracking-[0.3px]">
                    REF_C_{String(i + 1).padStart(3, "0")}
                  </span>
                </div>

                <div className="px-5 py-4 overflow-hidden">
                  <span className="text-[27px] text-fg-1 font-medium block overflow-hidden text-ellipsis whitespace-nowrap">
                    {loc?.nome_razao_social ?? "—"}
                  </span>
                </div>

                <div className="px-5 py-4 overflow-hidden flex flex-col gap-0.5">
                  <span className="text-[27px] text-fg-1 overflow-hidden text-ellipsis whitespace-nowrap">
                    {uni?.nome ?? "—"}
                  </span>
                  {edi && (
                    <span className="font-mono text-[15px] text-fg-4 overflow-hidden text-ellipsis whitespace-nowrap">
                ...
                <div className="px-5 py-4">
                  <span className="font-mono text-[27px] text-fg-3">
                    {fmtData(contrato.data_inicio)}
                  </span>
                </div>

                <div className="px-5 py-4">
                  <span className={cn("font-mono text-[27px]", expiring ? "text-warning" : "text-fg-3")}>
                ...
                <div className="px-3 py-4 flex flex-col gap-1.5 items-start">
                  <Button
                    ...
                    className="font-mono text-[15px] text-fg-3 uppercase tracking-[1px] font-bold p-0 h-auto"
                  >
                    VER →
                  </Button>
                  {isAtivo && !vencido && (
                    <Button
                      ...
                      className="font-mono text-[15px] text-danger-fg uppercase tracking-[1px] font-bold p-0 h-auto"
                    >
                      CANC
                    </Button>
                  )}
        ...
        <div className="flex justify-between items-center px-6 py-4 border border-border-3 text-fg-3">
          <span className="font-mono text-[17px] tracking-[0.5px]">
            Contratos encerrados são preservados como histórico imutável.
          </span>
          <Button
            variant="ghost"
            disabled
            className="font-mono font-bold text-[15px] tracking-[1.4px] text-fg-2 uppercase p-0 h-auto opacity-50 cursor-not-allowed"
          >
            Ver Arquivo →
          </Button>
        </div>
