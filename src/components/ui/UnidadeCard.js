  ...
  if (isEditing) {
    return (
      <div className="border-t border-border-3 px-5 py-5 bg-surface">
        <div className="mb-4">
          <span className="font-mono text-[17px] text-fg-5 tracking-[1px] uppercase">{refOf(unidade)}</span>
        </div>
        ...
        {erro && (
          <span className="font-mono text-[17px] text-danger-fg block mb-3">{erro}</span>
        )}
        ...
        <div className="flex gap-3">
          <Button
            ...
            className="bg-indigo text-fg-1 font-mono font-bold text-[20px] tracking-[1px] uppercase rounded-none px-5 h-9"
          >
            Salvar
          </Button>
          <Button
            ...
            size="sm"
            onClick={onCancelar}
            className="font-mono text-[15px] text-fg-3 uppercase tracking-[0.5px] font-bold h-9 px-4 rounded-none border border-border-3"
          >
            Cancelar
          </Button>
        </div>
    ...
    <div className="border-t border-border-3 px-5 py-4">
      <div className="flex justify-between items-start gap-3">
        <div className="flex flex-col gap-1 min-w-0">
          <span className="font-mono text-[17px] text-fg-5 tracking-[0.8px] uppercase">
            {refOf(unidade)}
          </span>
          <span className="font-display font-bold text-[27px] tracking-[-0.6px] text-fg-1">
            {unidade.nome}
          </span>
          {unidade.descricao && (
            <span className="font-body text-[27px] text-fg-3 mt-0.5 overflow-hidden text-ellipsis whitespace-nowrap">
              {unidade.descricao}
            </span>
          )}
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            {unidade.area_m2 && (
              <span className="font-mono text-[15px] text-fg-4 tracking-[0.5px] uppercase">
                {unidade.area_m2}m²
              </span>
            )}
            {unidade.valor_visivel ? (
              ...
              <span className="font-mono text-[15px] text-fg-5 tracking-[0.5px] uppercase">
                Valor sob consulta
              </span>
            )}
