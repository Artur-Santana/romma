  ...
  return (
    <div className="w-64 bg-background border-r border-[var(--border-1)] flex flex-col h-full shrink-0">
      ...
      <div className="px-8 pt-8 pb-6">
        ...
        <div className="font-display font-bold text-[42px] tracking-[-1px] text-fg-1 mt-2">
          ROMMA
        </div>
      </div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span
                className={cn(
                  "font-body font-bold text-[18px] tracking-[1.2px] uppercase",
              ...
              {badges[item.code] != null && (
                <span className="font-mono text-[15px] text-indigo">
                  {badges[item.code]}
                </span>
              )}
            </Link>
      ...
      <div className="border-t border-[var(--border-1)] px-8 pt-6 pb-8 flex flex-col gap-[10px]">
        <Link
          href="/"
          className="font-mono text-[17px] text-fg-3 no-underline tracking-[0.5px]"
        >
          → Ver Página Pública
        </Link>
        {email && (
          <span className="font-mono text-[15px] text-fg-5 mt-2 tracking-[0.5px]">
            {email}
          </span>
        )}
        <LogoutButton />
