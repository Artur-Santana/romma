...
export default function Unidades() {
    ...
    async function handleSalvarUnidade(id) {
        ...
        .from("unidades")
        .update({nome: nomeEdit, descricao: descricaoEdit, area_m2: area_m2Edit, valor_mensal: valor_mensalEdit, valor_visivel: valor_visivelEdit, status: statusEdit})
        .eq("id",editandoId)
        if (!error) {
            setEditandoId(null)
            setUnidades(await getUnidades())
    ...
    async function insertUnidade(e) {
        ...
        const { error } = await supabase
        .from("unidades")
        .insert({nome, descricao, area_m2, valor_mensal, status, valor_visivel, edificio_id})
        if (!error) {
            setUnidades(await getUnidades())
        }
    }
    ...
    return (
        <main>
            <form onSubmit={insertUnidade}>
                ...
                <input placeholder="nome" value={nome} onChange={(e)=> setNome(e.target.value)} type="text" ></input>
                <input placeholder="descricao"value={descricao} onChange={(e)=> setDescricao(e.target.value)} type="text" ></input>  
                <input placeholder="area_m2" value={area_m2} onChange={(e)=> setArea_m2(e.target.value)} type="number" ></input>
                <input placeholder="valor_mensal" value={valor_mensal} onChange={(e)=> setValor_mensal(e.target.value)} type="number" ></input>
                <select value={status} onChange={(e)=> {setStatus(e.target.value)}}>
                    <option key={"disponivel"} value={"disponivel"}>Disponivel</option>
                    <option key={"alugada"} value={"alugada"}>Alugada</option>
                </select>
                <label>Valor_visivel</label>
                <input checked={valor_visivel} onChange={(e)=> setValor_visivel(e.target.checked)} type="checkbox"></input>
