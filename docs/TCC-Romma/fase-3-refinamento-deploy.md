...
- [ ]  **C1.1** — Reescrever `supabase/functions/gerar-parcelas/index.ts`: extrair token do header `Authorization`, chamar `supabase.auth.getUser(token)`, verificar ownership do `contrato_id` antes de operar com service role.
...
- [ ]  **C1.3** — Modificar `src/lib/queries-server.js:67` e `src/lib/queries-client.js:60`: remover `valor_mensal` da projeção quando `valor_visivel = false`. RLS é filtro de linha, não de coluna — máscara visual no frontend é insuficiente.

**Tarefas — Eixo Auth & Rotas:**

- [ ]  **C2.1** — Criar `src/proxy.js` (middleware Next.js 16) usando `supabase-server.js`: validar sessão antes de renderizar qualquer rota `dashboard/**`. Redirect para `/login` se ausente.
- [ ]  **C2.2** — Em `src/app/login/page.js`: trocar import de `src/lib/supabase.js` por `src/lib/supabase-browser.js` (`createBrowserClient` de `@supabase/ssr`). Sem isso, sessão fica em `localStorage` e `proxy.js` (que lê cookies) não a enxerga → loop de redirect.
- [ ]  **C2.3** — Conectar `handleLogout` (definido em `src/app/dashboard/page.js:32`) a um botão de UI; em `Header.js`, adicionar condicional de auth: se sessão ativa, exibir avatar/logout em vez de "ENTRAR" / "COMEÇAR AGORA".
...
- [ ]  **C6.2** — Adicionar `!.env.example` ao `.gitignore` (após linha `.env*`) **antes** de criar o arquivo. Criar `.env.example` com `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_JWT`, `SUPABASE_ROLE_KEY` (sem valores).

**Verificação:**
- [ ]  POST não autenticado em `gerar-parcelas` retorna 401.
- [ ]  `curl https://<projeto>.supabase.co/rest/v1/locatarios` com anon key retorna `[]` ou 401.
- [ ]  `curl http://localhost:3000/dashboard` sem cookie de sessão retorna redirect para `/login`.
- [ ]  Aba Network: payload de `/unidades` para anônimos não contém `valor_mensal` quando `valor_visivel = false`.
- [ ]  Login → cookie de sessão presente em `document.cookie`; sessão sobrevive a refresh.
- [ ]  Botão de logout visível para autenticado; clique limpa cookie e redireciona.
