...
Página de gestão de Unidades com CRUD completo: select de edifício para filtrar, formulário com todos os campos do modelo (`nome`, `descricao`, `area_m2`, `valor_mensal`, `valor_visivel`, `status`), listagem com `.map()`, edição inline.

### `/src/app/dashboard/locatarios/page.js`

Página de gestão de Locatários com CRUD completo: convite por email, listagem, deleção e edição inline. Formulário com `nome_razao_social`, `tipo` (PF/PJ via select), `documento`, `email`, `telefone`. Edição inline usa `editandoId` + `editForm` (objeto único para todos os campos editáveis). Email não é editável pois é login do Auth.
...
Página de gestão de Contratos com CRUD completo: formulário com select de unidade e locatário + inputs de data e observações, listagem com edição inline, deleção. Lógica de negócio: criação atualiza unidade para `alugada`; deleção reverte para `disponivel`. Padrão `resetForm()` extraído como função separada. Integrado com Edge Function `gerar-parcelas` via `supabaseJWT.functions.invoke()` após insert do contrato.
